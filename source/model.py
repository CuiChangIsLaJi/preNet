...
class preNet_linear(nn.Module):
    def __init__(self,in_features,dropout,device=None):
        ...
        self.bn1 = nn.BatchNorm1d(in_features,device=device)
        self.linear1 = nn.Linear(in_features,100,device=device)
        self.bn2 = nn.BatchNorm1d(100,device=device)
        self.linear2 = nn.Linear(100,50,device=device)
        self.bn3 = nn.BatchNorm1d(50,device=device)
        self.linear3 = nn.Linear(50,1,device=device)
        self.drop = nn.Dropout(dropout)
    def forward(self,x):
        x = self.linear1(self.drop(self.bn1(x)))
        x = F.relu(x)
        x = self.linear2(self.drop(self.bn2(x)))
        x = F.relu(x)
        x = self.linear3(self.drop(self.bn3(x)))
        x = torch.sigmoid(x)
        return x
